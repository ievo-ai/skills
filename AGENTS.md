...
- **`--safe-mode` / `CLAUDE_CODE_SAFE_MODE` — total bypass.** Claude Code [v2.1.169](https://github.com/anthropics/claude-code/releases/tag/v2.1.169) added a `--safe-mode` CLI flag (and `CLAUDE_CODE_SAFE_MODE` env var) that starts Claude Code with **all** customizations disabled for troubleshooting: CLAUDE.md, plugins, skills, hooks, and MCP servers. iEvo is plugin-installed, so in safe mode every iEvo skill, hook, and sub-agent is silently absent — not degraded like the `CLAUDE_CODE_SUBAGENT_MODEL` gotcha below, entirely gone. `security-check/SKILL.md`'s `disallowed-tools` and the sub-agent tool isolation described above are moot because the skill/agent never loads. Listed first (severity order) because this is the most severe bypass documented in this section: total absence, not degraded reasoning. **Mitigation**: verify `CLAUDE_CODE_SAFE_MODE` is unset and `--safe-mode` was not passed before relying on `/ievo:security-check` or any other iEvo skill for security coverage; safe mode is intended for troubleshooting, not for environments where iEvo's guarantees are required.
