...
- **Codex sub-agent delegation — operator gotcha.** On Codex, `/ievo:init`, `/ievo:index-repos`, and `/ievo:security-check` rely on Codex's sub-agent tools (`spawn_agent`/`send_input`/`resume_agent`/`wait_agent`/`close_agent`) as the Codex-side equivalent of Claude Code's `Task` tool, for parallel security-auditor + repo-indexer dispatch. Per [Codex's subagents docs](https://developers.openai.com/codex/subagents) (verified 2026-07-09), **spawning defaults to explicit-request-only** — "Codex only spawns a new agent when you explicitly ask it to do so" — the opposite of Claude Code's always-proactive `Task` tool. Codex rust-v0.142.0 (2026-06-22) also added a more granular `multiAgentMode` (`none`/`explicitRequestOnly`/`proactive`; simplified in a later release), but that control lives at the **app-server protocol level** for IDE/programmatic integrations — it is not a documented Codex CLI flag or `config.toml` key, so don't assume a `/settings`-style toggle exists. **Silent-failure symptom**: the same docs note that in non-interactive flows, "actions requiring new approval fail if fresh approval cannot surface" — an unattended/batch iEvo run on Codex can have its parallel indexing + security-audit phase silently stall or return 0 results if a `spawn_agent` approval prompt has nowhere to surface. **Mitigation**: confirm `[features] multi_agent` is enabled (`~/.codex/config.toml`, or `codex --enable multi_agent`), and that the session/harness driving an unattended Codex run doesn't require interactive approval for `spawn_agent` calls before relying on these skills' parallel phase.
