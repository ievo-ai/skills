...
- **Sub-agent tool isolation.** A skill's `disallowed-tools` (kebab-case) does NOT propagate to a Task-tool-dispatched sub-agent, so `security-auditor.md` self-enforces with its own `disallowedTools` (camelCase per Claude Code sub-agent frontmatter) — blocking `Write`/`Edit`/destructive `Bash(rm*|mv*|cp*|curl*|wget*)` and `WebSearch`. WebSearch is denied because the auditor must never search the web about a candidate it is scanning: a target carrying prompt injection could turn that into an exfiltration channel. The auditor's `tools:` allowlist also drops `Write` entirely (it returns a verdict + report body in its output; the report Write happens in `/ievo:init` Step 8b, not in the agent). The security skills (`security-check`, `vuln-scan`, `deep-review`) carry the same `WebSearch` denial in their `disallowed-tools` and a `model: sonnet` turn-pin for direct invocation.
