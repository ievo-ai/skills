...
- **Codex MCP tool timeout — operator gotcha.** Codex rust-v0.141.0 (2026-06-18) raised the default MCP tool-call timeout from 120 to 300 seconds ([openai/codex#28234](https://github.com/openai/codex/pull/28234), merged 2026-06-15) — not from 60 seconds directly: the original 60s default ([openai/codex#3959](https://github.com/openai/codex/pull/3959), Sept 2025) was already bumped to 120s in Feb 2026 ([openai/codex#12405](https://github.com/openai/codex/pull/12405)) before #28234's 300s bump. `security-auditor` performs deep multi-file reads of every file in a candidate repo, which can exceed the shorter pre-v0.141.0 window on large repos and cut the scan off before it returns a verdict — the orchestrator can then proceed to install a candidate that was never fully audited. **Codex ≥ v0.141.0 is recommended for reliable parallel `security-auditor` dispatch.** Upgrade via `codex update` if running iEvo on Codex.
