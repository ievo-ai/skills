...
- **Codex plugin auto-update — stale skill-cache root mid-task (upstream bug, not iEvo-owned).** Codex Desktop's native plugin updater can install a newer iEvo release into a new versioned cache directory and prune the old one while a task that started under the previous version is still open. That already-open task keeps referencing the removed `<codex-cache>/ievo/<old-version>/skills` root, so invoking any `ievo:*` skill mid-task fails with `ENOENT` — even though `codex plugin list --json` correctly reports the newer version installed and enabled, and the new version's cache directory is present and readable on disk. Root cause lives entirely in Codex's own plugin-loader internals (`codex-rs`: `PluginsManager`/`SkillsManager` cache invalidation), not in any iEvo-owned code path — tracked upstream at [openai/codex#24390](https://github.com/openai/codex/issues/24390) (open as of 2026-07-25; a contributor traced it to `PluginsManager` caching `PluginLoadOutcome` without keying on the active plugin-cache version, and the curated-refresh path clearing only the `PluginsManager` cache, not `SkillsManager`, so a task can keep resolving a plugin-cache path already deleted from disk). **Mitigation**: start a new Codex task, or restart Codex Desktop, after an iEvo plugin auto-update completes — before invoking an `ievo:*` skill again in that task.
