...
└── plugins/ievo/                      # The plugin itself
    ...
    │   ├── scrub.mjs                  # Privacy scrub (stdin -> stdout) for auto-evolution failure-capture
...
**Current compliance ledger (v0.55.0):**
...
- ✅ `scrub.mjs` — 100 / 100 / 100. Same gate as above. Added in v0.55.0 (#422, opt-in tool-failure capture): pure stdin -> stdout privacy scrub called by the auto-evolution failure-capture hook before a captured record ever reaches disk — provider-shaped secret redaction, `*_TOKEN`/`*_KEY`/`*_SECRET`/`*_PASSWORD`/`*_ID` assignment redaction, `$HOME`-path rewrite, and a 500-code-point cap. Same `isCliEntry` / injected-io pattern as `evolution_candidates.mjs`.
