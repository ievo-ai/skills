...
- `disable-model-invocation: true` (optional, default `false`) makes a skill **user-invoke only** — its description is withheld from the model so it cannot auto-activate on description match. Reserve it for heavyweight skills (sub-agent dispatch, external calls) where accidental activation is costly; the current set is `init`, `security-check`, `vuln-scan`, `deep-review`. As of Claude Code v2.1.196 it also prevents a [scheduled task](https://code.claude.com/docs/en/scheduled-tasks) from firing the skill when the skill is the task's prompt. Explicit `/ievo:<name>` invocation is unaffected.
...
**Current compliance ledger (v0.36.0):**
