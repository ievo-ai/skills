...
- `paths` (optional, added v0.61.0, skills#157/#175) — glob patterns (YAML list) that scope auto-activation to sessions where a matching file is in context (`code.claude.com/docs/en/skills` — same format as memory path-specific rules). Unknown to platforms that don't support it; those ignore it gracefully rather than erroring, so it's safe forward-compat. Patterns are **root-anchored unless `**/`-prefixed** — the docs' own table reads `*.md` as "markdown files in the project root", so a bare `plugins/**` matches nothing in a project that doesn't vendor that directory at its root. Always write `**/plugins/**` for a directory that can appear at any depth. Only add it where file-context is genuinely predictive of relevance — currently `index-repos`, `hooks-setup`. Do NOT add it to always-relevant skills (`evo`, `handoff`, `feedback`, etc. — wrong gating is worse than none); the same reasoning excludes `vuln-scan`, whose CWE taxonomy is language-agnostic, so any extension allowlist silently kills auto-activation for every language it omits. Also do NOT add it to a skill that a sub-agent preloads via `skills:` frontmatter or loads through the skills system at the top of its run (`security-check` ← `evolution`, `security-auditor`): those consumers reach the skill before any candidate file is in context, and the docs neither confirm nor deny that the file-context filter is applied on the preload / sub-agent-invocation paths — so the gate would be an unverifiable risk of silently stripping a security audit, the same programmatic-consumer constraint the `disable-model-invocation` bullet above states for this skill. And do NOT add it to a skill that already carries `disable-model-invocation: true` (`deep-review`, `init`): that flag withholds the skill's description from the model entirely, so Claude never reaches the file-context check `paths` would gate, making the field silently inert there.
- `argument-hint` (optional, added v0.61.0, skills#157/#177) — hint text shown in the `/` menu autocomplete for skills that take positional arguments, e.g. `argument-hint: "[owner/repo] [ref]"`. Purely cosmetic autocomplete UX; ignored gracefully on platforms without `/` menu autocomplete. Current set: `inspect`, `feedback`, `index-repos`, `handoff`.
