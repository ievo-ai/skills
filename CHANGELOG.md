...
- **Fix** — added `paths:` to `index-repos` (skill/agent/plugin repo structure) and `hooks-setup` (`.claude/settings*` plus `**/SKILL.md` — settings files are the primary signal, but a session editing a SKILL.md is equally likely to want lifecycle hooks wired up). Every pattern that can appear at any depth is `**/`-prefixed: `paths` uses the same glob format as memory [path-specific rules](https://code.claude.com/docs/en/memory#path-specific-rules), whose own table reads `*.md` as "markdown files in the project root", so a bare `.claude-plugin/**` / `SKILL.md` would have matched nothing in any project that doesn't carry those at its root — dead gating, strictly worse than none. (`hooks-setup`'s `.claude/settings.json` / `.claude/settings.local.json` stay bare deliberately: those files only ever live at the project root.) **Deliberately excluded `deep-review`** despite its inclusion in the original #175 proposal: `deep-review/SKILL.md` already sets `disable-model-invocation: true`, which withholds the skill's description from the model entirely (verified against `code.claude.com/docs/en/skills`) — Claude never reaches the file-context check `paths` would gate, so the field would be silently inert there. **Also excluded `vuln-scan`**, proposed in #175 and gated in the first pass of this build: its CWE taxonomy is language-agnostic, so any extension allowlist (`**/*.ts`, `**/*.py`, `**/*.go`, …) is an allowlist that silently kills auto-activation for every language it omits — Rust, Java, Ruby, PHP, C# — and misses sibling extensions like `.tsx` besides. A security scanner failing to offer itself is the worst failure mode of a wrong gate, so the field was dropped rather than widened; the skill keeps its normal always-eligible description match. **Also excluded `security-check`**, gated in the earlier passes of this build: its two programmatic consumers reach it before any candidate file is in context — `evolution.md` preloads it via `skills:` sub-agent frontmatter for the Step 2.5 vendor-time re-audit (#357), and `security-auditor.md` Step 1 loads it through the skills system at the top of a fresh sub-agent, ahead of its own Step 2 clone/read. The docs define `paths` as limiting when Claude "loads the skill automatically", and preloading "draws from the same set of skills Claude can invoke", but neither page states whether the file-context filter applies on those two paths — an unverifiable risk of silently stripping the #357 antivirus gate, which is the same programmatic-consumer constraint AGENTS.md already records for `disable-model-invocation` on this skill. All three exclusions, plus the root-anchoring rule, are documented in AGENTS.md § Skills format next to the field's introduction.
- **Gap closed (#177)** — `inspect`, `feedback`, `index-repos`, and `handoff` accept positional arguments with no `argument-hint`, leaving the `/` menu autocomplete without a hint of expected input.
- **Fix** — added `argument-hint: "[owner/repo] [ref]"` (inspect), `"[title]"` (feedback), `"[owner/repo ...]"` (index-repos), `"[purpose]"` (handoff — its own body documents `/ievo:handoff <purpose>` and falls back to an `AskUserQuestion` prompt when invoked bare).
