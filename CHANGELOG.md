...
- **Fix** — added `paths:` to `security-check` (plugin/agent/marketplace files), `index-repos` (skill/agent/plugin repo structure), and `hooks-setup` (`.claude/settings*` plus `**/SKILL.md` — settings files are the primary signal, but a session editing a SKILL.md is equally likely to want lifecycle hooks wired up). Every directory pattern is `**/`-prefixed: `paths` uses the same glob format as memory [path-specific rules](https://code.claude.com/docs/en/memory#path-specific-rules), whose own table reads `*.md` as "markdown files in the project root", so a bare `plugins/**` / `agents/**` would have matched nothing in any project that doesn't vendor those directories at its root — dead gating, strictly worse than none. **Deliberately excluded `deep-review`** despite its inclusion in the original #175 proposal: `deep-review/SKILL.md` already sets `disable-model-invocation: true`, which withholds the skill's description from the model entirely (verified against `code.claude.com/docs/en/skills`) — Claude never reaches the file-context check `paths` would gate, so the field would be silently inert there. **Also excluded `vuln-scan`**, proposed in #175 and gated in the first pass of this build: its CWE taxonomy is language-agnostic, so any extension allowlist (`**/*.ts`, `**/*.py`, `**/*.go`, …) is an allowlist that silently kills auto-activation for every language it omits — Rust, Java, Ruby, PHP, C# — and misses sibling extensions like `.tsx` besides. A security scanner failing to offer itself is the worst failure mode of a wrong gate, so the field was dropped rather than widened; the skill keeps its normal always-eligible description match. Both exclusions, plus the root-anchoring rule, are documented in AGENTS.md § Skills format next to the field's introduction.
