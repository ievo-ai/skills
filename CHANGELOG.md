...
Add opt-in tool-call failure/denial capture for auto-evolution mode — `PostToolUseFailure`/`PermissionDenied` events become privacy-scrubbed `scope: tool-failure` evolution candidates — closes #422.

- **Gap closed** — auto-evolution's capture pipeline only ever heard USER corrections (`UserPromptSubmit`); the tool calls that actually fail or get denied mid-session — the largest source of ground-truth "where sessions stumble" signal — were never captured. A separate prototype proved the design end-to-end and also found the existing correction-capture/analysis-nudge hooks bake a *version-segmented* plugin-cache path into the generated scripts at enable time (`~/.claude/plugins/cache/<marketplace>/ievo/<version>/...`), which the next plugin update silently breaks (fail-open, so nothing surfaces that capture stopped) — see [anthropics/claude-code#15642](https://github.com/anthropics/claude-code/issues/15642).
- **New `plugins/ievo/scripts/scrub.mjs`** (Node, stdlib-only, stdin -> stdout, never writes a file) — the privacy scrub failure-capture pipes raw tool telemetry through before it ever reaches disk: provider-shaped secret redaction (the GitHub token family `ghp_`/`gho_`/`ghu_`/`ghs_`/`ghr_`/`github_pat_` — not just the two the issue named as examples, since a captured `ghs_`-shaped App installation token is exactly this repo's own CI shape; `sk-` OpenAI/Anthropic-style keys; Slack `xox[abprs]-` tokens; AWS `AKIA...` access key IDs; JWTs), `*_TOKEN`/`*_KEY`/`*_SECRET`/`*_PASSWORD`/`*_ID` assignment-value redaction (name kept, value redacted — the same convention AGENTS.md's public-repo-content-safety section already establishes), `$HOME`-absolute-path rewrite to `~`, and a hard 500-Unicode-code-point cap (never UTF-16 code units, so a truncation can't split a surrogate pair) with a trailing marker. Built on the `evolution_candidates.mjs` `isCliEntry`/injected-io pattern and covered to **100/100/100** by `tests/scrub.test.mjs`; registered in `.github/scripts/check-coverage.mjs` `REQUIRED`.
- **`evo-auto-enable/SKILL.md`** — Step 1.5 (new) asks, once, at fresh-enable time via `AskUserQuestion`, whether to also capture tool-call failures/denials; the answer is written as `.ievo/evo-auto.flag`'s `signal` field (`corrections-only`, unchanged default, or `corrections+failures`). Idempotent re-enable keeps the existing signal rather than re-asking. When `corrections+failures`, Step 3.6 (new) writes `.ievo/hooks/scripts/failure-capture.sh` — gated on flag + signal, extracts `tool_name`/`error`/`tool_input` via `jq`, pipes the raw detail through `scrub.mjs`, and appends via `evolution_candidates.mjs append --scope tool-failure` (the `--scope` flag already existed — zero accumulator changes). Pure observer contract: always exits 0, no stdout, fail-open for the pipeline but fail-CLOSED for content (a scrub error drops the record rather than risk writing raw text). Step 3.7 wires the same hook entry under both `hooks.PostToolUseFailure` and `hooks.PermissionDenied`. The SessionStart nudge (Step 3.5.3) now also tells the reviewer to apply failure-then-fixed-vs-noise judgment to any `scope: tool-failure` candidate before folding it in.
- **Path-resolution fix (all three generated scripts)** — `correction-capture.sh`, `evo-analysis-nudge.sh`, and the new `failure-capture.sh` no longer bake any path at enable time. Each embeds a small `resolve_ievo_script()` shell function that prefers a live `CLAUDE_PLUGIN_ROOT` and otherwise globs the plugin's cache root fresh **on every hook fire** — no version number ever appears literally in a generated script, so a plugin update is picked up on the very next fire instead of chasing a stale baked path. Closes the drift class the prototype found dead in the wild.
- **`evo-auto-disable/SKILL.md`** — Step 3.5 now also removes the failure-capture hook entries (from both `hooks.PostToolUseFailure` and `hooks.PermissionDenied`) and deletes `failure-capture.sh`, unconditionally and idempotently (a no-op if failure capture was never enabled).
- **Out of scope (deliberately)** — repeat/dead-end (loop) detection is a stretch follow-up; no Eva-side consumer is added (auto-evolution stays local-first by design — records batch into the next `/ievo:evo` review, same as corrections).
- **Scope** — confined to `plugins/ievo/scripts/scrub.mjs` (+ tests), `evo-auto-enable/SKILL.md`, `evo-auto-disable/SKILL.md`, and this ledger/changelog. No changes to `evolution_candidates.mjs` behavior (only its `SCRIPT_VERSION` bumps, coupled to `plugin.json`).
- **Version** — bump per AGENTS.md rules (`feat:` → minor); `discover.mjs` and `evolution_candidates.mjs` `SCRIPT_VERSION`, `plugin.json`, `marketplace.json`, and the AGENTS.md compliance ledger updated in lockstep. `scrub.mjs` ships at `SCRIPT_VERSION` `0.55.0`, coupled to `plugin.json` from the start.
