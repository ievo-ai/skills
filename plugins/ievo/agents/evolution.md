...
- **Neutralize the symlink-containment and re-audit SKIPPED lines before they render.** Step 5's symlink-containment `SKIPPED` line and its re-audit `SKIPPED` line both interpolate an `<owner>/<repo>@<path>` pointer — a tree path that can hold almost any byte — and the re-audit line also interpolates LLM-synthesized flag text; both are rendered as Markdown by whatever session/skill dispatched this agent. See Step 5's "Excerpt containment" note for the fencing rule covering all of it. Step 5's third `SKIPPED` line (Step 3.5's third-party-excerpt refusal) needs no such fencing — its `<the signal that triggered this>` field is a bounded category label, not a verbatim quote of untrusted lesson text.
