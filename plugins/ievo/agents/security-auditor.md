...
**Operator notice — `CLAUDE_CODE_SUBAGENT_MODEL` override**: Claude Code v2.1.144+ propagates this environment variable to subagent teammates. If your project or CI sets `CLAUDE_CODE_SUBAGENT_MODEL=haiku` (e.g. to reduce cost on low-stakes tasks), it will silently override this agent's `model: sonnet` declaration, downgrading security scans to Haiku tier. Haiku **misses indirection attacks** — a real supply-chain threat class. Recommendation: exclude the security-auditor from cost-reduction overrides, or confirm via the OTEL `x-claude-code-agent-id` spans that the active model is Sonnet before trusting verdicts.
