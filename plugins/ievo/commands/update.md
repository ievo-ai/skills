...
5. **Skill** (`source.path` is the skill's directory): enumerate the now-verified `$CHECKOUT_DIR/<source.path>` with the **Glob tool** (`pattern: "**/*"`, `path: "$CHECKOUT_DIR/<source.path>"` — never a Bash `find`/`ls`). **Before reading any enumerated entry, verify none of them is a symlink.** Sub-step 3's containment check runs once, on the directory named by `source.path` — it says nothing about what upstream placed *inside* that directory. A symlink planted in the cloned tree (upstream is, or later becomes, attacker-controlled) can point anywhere on the runner's filesystem — e.g. a local credential file — and would be followed transparently by the Read tool, staging its resolved target's bytes under a name that looks like an ordinary skill file. Run `find "<canonical-checkout-dir>" -type l` to detect it: this embeds only sub-step 3's canonical form of `$CHECKOUT_DIR` (`mktemp`-generated, then resolved against the real filesystem — `<source.path>` contributes nothing to it, so it stays never-attacker-controlled) in the command line — never `<source.path>` or any enumerated filename — so it carries none of the shell-injection risk sub-step 3 warns against. Rooting `find` at the canonical checkout dir rather than the raw `$CHECKOUT_DIR` string is what makes the comparison below purely lexical: `find` prints its starting path exactly as it was given and appends the real directory components it walked (it never follows a symlink — it reports it and stops there), so every path it returns is already in the same canonical terms as `<canonical-source-dir>`. Treat the command's output as plain data (never re-embed a returned path into another Bash command) and check whether any returned path *is* `<canonical-source-dir>` or begins with `<canonical-source-dir>` plus a `/` — a plain string-prefix comparison, the same "check the string, don't shell out with it" style as sub-step 3. **Compare against `<canonical-source-dir>`, never against the raw `$CHECKOUT_DIR/<source.path>` string.** `source.path` is attacker-controlled frontmatter: a value like `skills/./foo` or `foo/../foo` passes sub-step 3's containment unchanged, yet matches nothing `find` ever prints — the prefix test would find no symlink to complain about and wave the target through, while Glob, which resolves its `path` parameter before enumerating, still walks that very directory and Reads every nested symlink under it. If any returned path does match, refuse the whole target — stop before Read/Write touches anything in this directory, same disposition as a sub-step 3 containment failure (see below). Otherwise, Read each listed file and Write it to the matching relative location under `<stage-dir>/<name>/`.
...
- **`source.path` containment, not just shell-safety.** Unlike `<source-path-in-repo>` in the sibling fetches (derived by walking a real cloned tree, which git itself never lets contain a bare `..` component), `update.md`'s `source.path` is raw overlay frontmatter an attacker can set to anything. Step 2 sub-step 3 resolves `$CHECKOUT_DIR/<source.path>` and confirms it stays inside `$CHECKOUT_DIR` before Glob/Read ever use it — a `../`-laden value must be refused, not merely passed through as "safe because it's a tool parameter". Containment doesn't stop at the directory boundary either: sub-step 5 checks every entry a skill's Glob enumeration returns for a nested symlink, since upstream content placed *inside* an already-verified directory is just as untrusted as the directory's own name — a symlink there can point outside `$CHECKOUT_DIR` regardless of how clean `source.path` itself is.
