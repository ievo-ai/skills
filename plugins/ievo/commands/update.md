...
3. **Verify `source.path` stays inside the clone before Glob/Read ever touch it.** `source.path` is raw frontmatter text, not a path obtained by walking the cloned tree — a crafted overlay can set it to `../../../../etc/passwd` or similar. Compute the two named values below and **carry both forward**, substituted literally wherever they appear later, the same convention as `<stage-dir>` above. Sub-steps 4/5/6 are defined in terms of them, and the raw `$CHECKOUT_DIR/<source.path>` string is never a substitute for `<canonical-source-dir>` — nothing below may consume it:
   - **`<canonical-checkout-dir>`** — `$CHECKOUT_DIR` resolved against the real filesystem, symlinks included. This one *may* cross a shell (`(cd "$CHECKOUT_DIR" && pwd -P)` — POSIX, no `realpath` dependency; record the printed path against this target's `<name>`), because `$CHECKOUT_DIR` is `mktemp`-generated and never attacker-controlled. Resolving it matters: on macOS `mktemp -d` returns `/var/folders/...` while the real path is `/private/var/folders/...`, and every comparison below would otherwise be rooted on a string the filesystem never echoes back.
   - **`<canonical-source-dir>`** — `<canonical-checkout-dir>` joined with `<source.path>`, normalized **purely lexically** into a single normal form, in this order: collapse every run of consecutive `/` down to one, drop `.` components, resolve `..` textually against the components to its left, then strip any trailing `/` (never shortening the value past `<canonical-checkout-dir>` itself — a `source.path` of `.` or the empty string normalizes to exactly the checkout dir, which sub-step 3's containment allows as the "equal to it" case). The two slash rules are not cosmetic tidying; they are load-bearing for sub-step 4, whose tests are plain string comparisons against paths `find` prints, and `find` prints exactly one `/` between components and none at the end. Without them a `source.path` of `skills/foo/` or `skills//foo` yields a `<canonical-source-dir>` no `find` output can ever match — against `<root>/skills/foo/`, the printed `<root>/skills/foo/evil-link` is not equal to it, is not an ancestor of it, and is not prefixed by it plus a `/` (that would require the impossible `<root>/skills/foo//`) — so all three tests come up clean and the target is waved through, while Glob/Read normalize the very same path themselves and walk that directory anyway. One trailing byte of attacker-controlled frontmatter would otherwise reopen the CWE-59 hole sub-step 4 exists to close. (Despite the name it is a *file* path in the Agent branch and a *directory* path in the Skill branch; both branches use the same value.) This canonicalization deliberately does **not** resolve symlinks, and no step may upgrade it to one that does — resolving them means handing the still-unvalidated `<source.path>` to the filesystem, which under this file's rules can only happen through a Bash command line and would reopen the CWE-78 hole this fetch protocol exists to close. Lexical `..` resolution is also the stricter containment answer in its own right: `realpath`-style resolution treats `link/..` as the *link target's* parent, which is the classic way a `..` walks out of a tree a textual check would have kept it inside.

   Confirm `<canonical-source-dir>` is `<canonical-checkout-dir>` itself or a descendant of it — equal to it, or beginning with it plus a `/`. Do this as a plain path check on the string already in hand from Step 1 — never by writing `<source.path>` into a Bash command line to test it, which would reopen the same CWE-78 this fix closes (it is still unvalidated for shell metacharacters). If containment fails, refuse the target — report `SKIPPED — invalid source metadata` in Step 6, same as a Step 1 validation failure.

   Containment is not the whole check, *precisely because* this form is lexical: a path that is textually inside the clone can still be, or traverse, a symlink pointing anywhere on the runner's filesystem. Sub-step 4 closes that dimension — for both branches, not just the Skill one.
4. **Verify no symlink lies on, or under, the path about to be read — this gates both branches below.** Sub-step 3 settled where `<canonical-source-dir>` points *textually*; it says nothing about whether a component of that path is a symlink, nor about what upstream placed *inside* it. A symlink in the cloned tree (upstream is, or later becomes, attacker-controlled) can point anywhere on the runner's filesystem — e.g. a local credential file — and both the Read tool (sub-step 5) and the Glob tool (sub-step 6, which resolves its `path` parameter before enumerating) follow one transparently, staging its resolved target's bytes under a name that looks like ordinary upstream content (CWE-59). Detect it with:
   ```bash
   find "<canonical-checkout-dir>" -type l
   ```
   This embeds only sub-step 3's resolved form of `$CHECKOUT_DIR` (`mktemp`-generated, then resolved against the real filesystem — `<source.path>` contributes nothing to it, so it stays never-attacker-controlled) in the command line — never `<source.path>`, never an enumerated filename — so it carries none of the shell-injection risk sub-step 3 warns against. Rooting `find` at the canonical checkout dir rather than the raw `$CHECKOUT_DIR` string is what makes the comparison below purely lexical: `find` prints its starting path exactly as it was given and appends the real directory components it walked (it never follows a symlink — it reports it and stops there), so every path it returns is already in the same terms as `<canonical-source-dir>`.

   Treat the output as plain data (never re-embed a returned path into another Bash command) and refuse the whole target if any returned path `P` is any of:
   - **the target itself** — `P` equals `<canonical-source-dir>`: the agent `.md` file, or the skill directory, is itself a symlink.
   - **an ancestor of it** — `<canonical-source-dir>` begins with `P` plus a `/`: a directory component along the way is a symlink. `find` stops at it and never prints anything beneath, so no other test would flag it, yet Read/Glob resolve straight through it.
   - **nested inside it** — `P` begins with `<canonical-source-dir>` plus a `/`: a symlink planted among the files a skill's Glob enumeration returns.

   All three are plain string comparisons, the same "check the string, don't shell out with it" style as sub-step 3. **Compare against `<canonical-source-dir>`, never against the raw `$CHECKOUT_DIR/<source.path>` string.** `source.path` is attacker-controlled frontmatter: a value like `skills/./foo`, `foo/../foo`, `skills/foo/`, or `skills//foo` is contained either way, yet in its raw form matches nothing `find` ever prints — every comparison above would find no symlink to complain about and wave the target through, while Read and Glob resolve the path and walk that very directory anyway. Sub-step 3's normal form is exactly what makes these comparisons meaningful, which is why it collapses repeated slashes and strips a trailing one as well as resolving `.`/`..` — each of those four values normalizes to a slash-collapsed, trailing-slash-free string in exactly the terms `find` prints (the three `skills/...` forms all to `<root>/skills/foo`). On a match, refuse the target before sub-step 5/6 Reads anything — same `SKIPPED — invalid source metadata` disposition as a sub-step 3 containment failure (see below).
5. **Agent** (`source.path` is the single agent `.md` file, not a directory — Glob-enumerating a file path returns nothing): Read the now-verified `<canonical-source-dir>` directly with the **Read tool**, then Write its content to `<stage-dir>/<name>.md` with the **Write tool**.
6. **Skill** (`source.path` is the skill's directory): enumerate the now-verified `<canonical-source-dir>` with the **Glob tool** (`pattern: "**/*"`, `path: "<canonical-source-dir>"` — never a Bash `find`/`ls`), then Read each listed file and Write it to the matching relative location under `<stage-dir>/<name>/`.
...
- **Restore executable permissions after fetch.** If a skill has `scripts/` with executable files, restore permissions once the staged content is applied (`chmod +x` on `.sh`/`.py` known patterns). Symlinked content needs no ordering rule here: Step 2 sub-step 4 refuses any target with a symlink on or under the fetched path outright, so no symlink ever reaches staging.
...
- **`source.path` containment, not just shell-safety.** Unlike `<source-path-in-repo>` in the sibling fetches (derived by walking a real cloned tree, which git itself never lets contain a bare `..` component), `update.md`'s `source.path` is raw overlay frontmatter an attacker can set to anything. Step 2 sub-step 3 resolves `$CHECKOUT_DIR/<source.path>` and confirms it stays inside `$CHECKOUT_DIR` before Glob/Read ever use it — a `../`-laden value must be refused, not merely passed through as "safe because it's a tool parameter". That resolution is deliberately **lexical** (no symlink resolution — the unvalidated `source.path` must never reach the filesystem through a shell), so containment settles only where the path points textually. Sub-step 4 covers the rest, for **both** the Agent and Skill branches: a single `find "<canonical-checkout-dir>" -type l` refuses the target if any symlink *is* the verified path, is an ancestor component of it, or is nested inside it — upstream content placed on or under an already-verified path is just as untrusted as the path's own name, and a symlink anywhere along it can point outside `$CHECKOUT_DIR` regardless of how clean `source.path` itself is.
