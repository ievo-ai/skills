...
`exploit_chain.entry/flow/impact`, `recommendation`, `file`, and
`preconditions` values may carry a
verbatim source excerpt (or an untrusted file path) from the scanned module, already wrapped in a
backtick code span by the `vuln-scanner` agent (see its "Excerpt
containment" rule) — specifically to stop a crafted `![...](...)`/
`[...](...)` in scanned source, or in a scanned file's own name, from rendering as a live exfiltration beacon
