...
Claude Code [v2.1.161](https://github.com/anthropics/claude-code/releases/tag/v2.1.161) (released 2026-06-02, verified 2026-07-26) started including `OTEL_RESOURCE_ATTRIBUTES` values as labels on every metric datapoint, so usage can be sliced by custom dimensions — that release's notes lead with "`OTEL_RESOURCE_ATTRIBUTES` values are now included as labels on metric datapoints, so you can slice usage metrics by custom dimensions like team or repo". The release note is the source of that version floor: the [monitoring docs](https://code.claude.com/docs/en/monitoring-usage) state the behaviour as current ("Claude Code attaches these values as attributes on every metric datapoint and event record, in addition to sending them in the OTLP resource block") with no `min-version` marker, so the floor can't be corroborated from the docs page alone. The value is read once, at process start, so export it **before launching Claude Code** to tag that whole session's token usage:
