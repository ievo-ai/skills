...
`-z` is required too, for a failure that is harder to see. `core.quotePath` defaults to **on**, so without `-z` git C-quotes every path containing a non-ASCII byte, a double quote, a backslash, or a control character: `café.md` comes back as the literal `"caf\303\251.md"` — surrounding quotes and octal escapes included. Fed to the command below, that name matches nothing on disk, so `git diff --no-index` writes `error: Could not access ...` to stderr and **exits 1 — the same status it returns on a successful difference** (verified on git 2.54.0). Exit code alone therefore cannot distinguish the failure from the expected case, and the file silently gets zero coverage: #483's own bug class, reached through a filename rather than through tracking state. `-z` disables that quoting entirely and NUL-terminates each path instead.

`| tr '\0' '\n'` is required for the same reason `-z` is, and neither substitutes for the other. `-z`'s NUL terminators cannot be read back: raw NUL bytes do not survive tool output — they arrive collapsed to spaces (verified: `printf 'a\0b\0'` reads back as `a b`), so every path in the listing merges into one space-joined string, indistinguishable from a single filename containing spaces. That joined string matches no file, so the synthesized diff below fails and *every* untracked file gets zero coverage — #483's bug class again, and worse than the quoting bug it replaced, since it fires on any tree with two or more untracked paths rather than only on non-ASCII names. `tr` converts each NUL back to a newline, so **split the listing on newline** and carry each path through verbatim; `-z` still does its own job, suppressing the C-quoting, because it governs how git *writes* the path and `tr` only changes the separator. One residual case is accepted rather than papered over: a filename containing a literal newline splits into fragments here. It does not fail silently — each fragment matches no file, so its `git diff --no-index` prints `error:` with no diff body, which the rule below records as a skipped `could not be read` path in the truncation notice.

For each path returned — subject to the caps below, and resolved from the repo root, since `--full-name` makes the paths root-relative rather than cwd-relative, and shell-quoted, since `-z` strips git's own quoting but escapes nothing for the shell, and an unquoted path still breaks on filenames containing spaces or shell metacharacters:

```bash
git -C "$(git rev-parse --show-toplevel)" diff --no-index -- /dev/null "<path>"
```

This prints a standard "new file" unified diff (`--- /dev/null` / `+++ b/<path>`, with `<path>` root-relative, matching `git diff`'s own headers) and exits 1 — expected, the same nonzero exit `git diff --no-index` always returns when a difference exists, not an error. Because that same exit 1 is what an access failure returns, never read the status alone as success: an invocation that printed `error:` and produced no diff body is a *failed* capture, not a covered file, so record it among the skipped paths below rather than passing it off as reviewed. With `-z` the quoting cause is gone, but a fragment of a newline-bearing filename, or a file deleted between the listing and the diff, still lands here. `--exclude-standard` respects `.gitignore`, so ignored files stay excluded, matching every other mode's git-tracked-or-intentionally-untracked scope; a binary untracked file reports `Binary files /dev/null and b/<path> differ`, same as git already does for binary changes elsewhere. Append each generated diff to the `git diff` output captured above — the combined text is the working-tree diff for Step 4. Nothing is staged: `git status` still reports these paths as untracked afterward.
...
**A capped run must never read as full coverage.** Record every skipped path and why (`over the 50-path cap` / `over the 256 KB budget` / `could not be read`), and carry that record to both consumers — an untracked file that was never reviewed has to stay distinguishable from one that was reviewed and found clean:
