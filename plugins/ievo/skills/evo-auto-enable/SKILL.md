...
**Security precondition — gitignore `.ievo/hooks/` BEFORE vendoring.** The
`scrub.mjs` copied below is the privacy-redaction engine itself. The old baked
fallback pointed at the plugin cache *outside* the repo, so it was never
committable; a project-local copy under a **git-tracked** `.ievo/hooks/` could be
committed and then altered in a PR to defeat scrubbing — a real escalation this
vendoring introduces. So before copying anything, ensure `.ievo/hooks/` is
git-ignored — a security gate here, not the convenience offer Step 3.5.4 makes
for the scripts. Check with `git check-ignore -q .ievo/hooks/` (exit 0 = already
ignored, the normal `/ievo:init` Step 10 state). If it is NOT ignored (a git repo
where init never ran), append `.ievo/hooks/` to the project's `.gitignore` NOW,
before the copy below — creating `.gitignore` if the git repo lacks one. Only
skip this when the project is not a git repo (`git rev-parse --is-inside-work-tree`
fails — nothing to track, so nothing PR-tamperable). Never vendor `scrub.mjs`
into a location git would track.
...
these generated scripts are covered by that same guard. Because the scripts
