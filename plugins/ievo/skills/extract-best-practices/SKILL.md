...
- **GREEN** → perform Registration's write, then include this candidate at CHECKPOINT 2.
- **YELLOW or RED** → do NOT write. Ask via `AskUserQuestion` first: `<type>/<name> was flagged <verdict> on re-audit: <top 1-2 flags — category + one-line explanation>. Author it anyway?` — options `Author anyway (I've reviewed the flags)` (write, then include at CHECKPOINT 2) or `Discard — do not author this candidate` (skip the write entirely; report the candidate as discarded, same end state as if the user had picked Option C — Skip — at CHECKPOINT 1).
...
- Never fabricate a lower verdict, and never write or present a YELLOW/RED candidate without an explicit override (a headless run's auto-selection is always a discard, never an override). Because the audit precedes the write, a discard needs no delete — nothing reached disk, so no capability beyond this skill's declared Glob/Read/Write surface is required.
