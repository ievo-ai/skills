...
1. **Flow A only.** Skip entirely in flow **(B)** (rejections — the type is Idea, not a bug) and in flow **(C)** (this feedback was itself pre-filled by a caller — `/ievo:evo` Step 5.6 or Step 5.65, or `/ievo:extract-best-practices` Phase 5). The flow-C skip is the **loop guard**: a bug captured via evo → feedback must not immediately bounce back feedback → evo. It also breaks the forward loop — a handoff from this step lands in `evo`, whose own Step 5.6 (or, when 5.6 classified the lesson local and asked nothing, Step 5.65) may offer feedback again as flow **(C)**, where this step is skipped, so the bridge always terminates.
...
- `evo` then runs its **Steps 1–5.7 unchanged**: scope confirmation (Steps 1 / 1.5), overlay append (Step 4), one-time marker injection, its own upstream-escalation offers (Step 5.6, and Step 5.65 when 5.6 asked nothing), and its Step 5.7 cluster-extraction offer — same "the receiving skill still runs its own gates" pattern the reverse bridge uses. Nothing about this skill's public issue is affected.
