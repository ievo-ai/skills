...
2. If not present, scan the invoking client's own load paths for installed skills via Glob (detect per `/ievo:init` Step 1.5 — `$CODEX_CLI` set, or a Codex Desktop signal when unset; same rule as `evo/SKILL.md` Step 1 — scanning the other client's dirs would suggest skills the next session can't actually load):
...
Detect the active platform via env-var signals ONLY — `/ievo:init` Step 1.5's canonical rule, the same one Step 2d uses, never `codex --version`/`claude --version` (those only prove a CLI is installed alongside the current one, not which platform is actually driving this session). Step 1.5 is `$CODEX_CLI` set, **or** — when it is unset — a Codex Desktop signal (`CODEX_INTERNAL_ORIGINATOR_OVERRIDE=Codex Desktop`, or macOS `__CFBundleIdentifier=com.openai.codex`); keying off the bare `$CODEX_CLI` var alone would run `claude plugin list` in a Codex Desktop session (issue #461):
