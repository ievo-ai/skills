...
- All matching entries `"enabled"` literally `true` → the plugin layer is loaded,
  ...
  lists an overlay for what Step 9a just vendored; only if it doesn't is the
...
- Command succeeded but no entry matched, or the match's `enabled` is not a
  ...
  `/ievo:overlay-status` first, and only if that too shows no overlay layer treat
  the install as genuinely incomplete (re-run Step 9, or check `.claude/skills/`
  for a path conflict).
