...
The script:
1. Builds 15-30 queries from the stack for a typical project (language fundamentals + per-dep + per-category + stack-specific compound + a fixed stack-independent group for general-purpose codebase-audit/planning-advisor meta-tools — not gated behind any detected category, since that class of skill isn't tied to a language/framework/dep), **hard-capped at 100** (`MAX_QUERIES`, #701/CWE-400). A stack big enough to derive more than 100 — a large polyglot monorepo, or an attacker-influenced payload — gets a max-min fair share across the six query layers and sets `queries_capped: true` in the output (step 5 below; handle it in Step 5c).
...
**Also read `queries_capped`.** If the discover output has `queries_capped: true`, the stack tripped the 100-query cap, so discovery ran on a fair share of the derived queries rather than all of them and `candidates[]` is a subset of what an uncapped run would have returned. Record it on section 5's "Queries generated" line ([log-format.md §5](references/log-format.md)) and say so in the run summary — e.g. "discovery was capped at 100 queries for a stack this large; the candidate list may be incomplete" — the same way a partial-failure run (Step 5b1) is surfaced. A cap that trips is never silent: proceed with the capped candidates, but don't let the user read the result as exhaustive.
