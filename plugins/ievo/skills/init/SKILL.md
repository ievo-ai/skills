...
Each `repo-indexer` writes to `<project>/.ievo/cache/index/<owner>-<repo>-<hash>.md` (no conflicts — different paths per repo, hash-suffixed so colliding slugs can't overwrite each other).
