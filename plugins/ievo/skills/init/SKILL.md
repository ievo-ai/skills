...
mechanical checks: **A** re-reads `.claude/settings.json` to confirm every
`enabledPlugins` write this run made landed (iEvo's own Step 2.2 entry plus all
`<K>` of Step 9b's plugins); **B** reads what the client has already loaded, via
the `claude plugin list --json` shell form — never the interactive `/plugin`
slash form, whose menu this skill can neither drive nor read back. Neither is
sufficient alone: the `<K>` plugins are invisible to B until the reload, and the
plugin registry sees nothing under `.claude/skills/`, so both paths end in the
same read-only `/ievo:overlay-status` smoke test — itself gated on this run
having vendored something (`<N>` + `<M>` > 0). Every unexpected reading degrades
read-only rather than re-running a mutating step. Full protocol incl. the exact
commands, entry shape, per-branch handling, scope/id detection, and the
vendor-count gate:
**[post-install-verification.md](references/post-install-verification.md)**.
