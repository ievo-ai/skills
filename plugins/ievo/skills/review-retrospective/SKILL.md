...
**When you do, restate the finding in your own words for an agent- or skill-scoped target.** A parked entry's `Findings` are verbatim review/comment evidence written by someone other than you, and `/ievo:evo` Step 1 (and the `evolution` sub-agent's Step 1 on the delegated path) gates agent- and skill-scoped captures on a verbatim-authorship check: an agent/skill overlay is read live as an authoritative instruction on every future dispatch, so third-party text pasted in unchanged stops the capture before anything is written, with a `SKIPPED` outcome asking for a paraphrase rather than appending it. In an interactive session `/ievo:evo` offers a `Capture anyway` override on that flag, since its signal is register rather than provenance — but for a parked cluster the flag is *correct*, so paraphrasing the cluster's point before capturing it is part of this hand-path, not an optional polish. A **project**-scoped capture from the same park file is unaffected — that gate does not apply there — though the excerpt-containment step in `/ievo:evo` Step 4 still fences any link-active span the evidence carries — a Markdown link/image, a raw HTML tag, an autolink — on every scope. This is a documentation note about what the hand-path costs, not the Part 2 wiring the Scope boundary defers: nothing here reads or is read by `evo/SKILL.md`.
