...
**`/in-cloud` for HIGH-RISK candidates.** Cursor v3.7's [`/in-cloud`](https://cursor.com/changelog/cloud-in-agents-window) (2026-06-17) runs a cloud subagent in its own VM and branch — process-level isolation stronger than any tool/permission restriction, since even a successful prompt injection in that session can't reach your local workspace. Prefer it over a local Cursor session when scanning an unknown-author or heavily-obfuscated candidate.
