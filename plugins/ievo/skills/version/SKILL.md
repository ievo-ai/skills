...
- **Confidently CLI, or uncertain** — detect the install scope first, then render the `claude` CLI update command (never the bare `/plugin update ievo` slash form: the built-in `/plugin` command's own documented direct-acting subcommands are `list`, `install`, `enable`, and `disable` — `update` is not among them, so `/plugin update <name>` isn't documented to act on its arguments the same way).
