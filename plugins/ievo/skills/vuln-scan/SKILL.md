...
Before reading files in Step 1, run a Glob pass over `module_path` to pre-flag paths that commonly hold real credentials — a cheap head start for the redaction rule below, not the sole trigger for it (see Rules § "Never echo raw secret values": that obligation covers any real secret value, wherever it turns up, not only files matched here).
